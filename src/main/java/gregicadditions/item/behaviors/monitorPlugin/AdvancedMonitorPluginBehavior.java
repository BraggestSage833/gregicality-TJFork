...
public class AdvancedMonitorPluginBehavior extends ProxyHolderPluginBehavior {
    ...
    @SideOnly(Side.CLIENT)
    private Map<BlockPos, Tuple<Vector3f, List<MetaTileEntityMonitorScreen>>> connections;
    @SideOnly(Side.CLIENT)
        ...
        worldSceneRenderer.setAfterWorldRender(()->{
            if (connect && connections != null) {
                for (BlockPos pos : connections.keySet()) {
                    ...
                    connections.get(pos).setKey(winPos);
                    if(winPos != null) {
    ...
    public void setConfig(float scale, int rY, int rX, int rZ, float spin, boolean connect){
        if(this.scale == scale && this.rY == rY && this.rX == rX && this.rZ == rZ && this.spin == spin && this.connect == connect) return;
        if(scale < 0.3 || scale > 2 || rY < 0 || rY > 360 || rX < 0 || rX > 360 || rZ < 0 || rZ > 360 || spin < 0 || spin > 2) return;
        this.scale = scale;
        this.rY = rY;
        this.rX = rX;
        this.rZ = rZ;
    ...
    public void renderPlugin(float partialTicks, RayTraceResult rayTraceResult) {
        if (worldSceneRenderer != null && this.screen != null) {
            ...
            if(this.connect && connections != null) {
                ...
                for (Tuple<Vector3f, List<MetaTileEntityMonitorScreen>> tuple : connections.values()) {
                    ...
                    if (origin != null) {
                        ...
                        RenderHelper.renderRect(oX, oY, 0.05f, 0.05f, 0.002f, 0XFFFFFF00);
                        for (MetaTileEntityMonitorScreen screen : screens) {
                            ...
                            RenderHelper.renderRect(dX, dY, 0.05f, 0.05f, 0.002f, screen.frameColor);
                            RenderHelper.renderLine(oX + 0.025f, oY + 0.025f, dX + 0.025f, dY + 0.025f, 0.01f, screen.frameColor);
                        }
                    }
